# Fine-tune BrainLM (vandijklab/brainlm) on raw fMRI timeseries — encoder
# fine-tune (full / linear-probe / last-k) + linear classification head.
#
# Supports: abide | parkinson  ONLY.
#
# Usage:
#   python finetune_brainlm_timeseries.py --dataset abide
#   python finetune_brainlm_timeseries.py --dataset parkinson --seeds 42 43 44 45 46
#   python finetune_brainlm_timeseries.py --dataset parkinson_a424 --seeds 42 43 44 45 46

#   python finetune_brainlm_timeseries.py --dataset parkinson --finetune_mode linear
#   python finetune_brainlm_timeseries.py --dataset parkinson --finetune_mode last_k --unfreeze_last_k 2
#   python finetune_brainlm_timeseries.py --dataset abide --pooling mean
...
from finetune_llama import (
    DATASET_CFG, get_split, seed_everything,
    youden_threshold, compute_metrics, count_params,
)


# ===== Data loading (raw timeseries, not static FC) =====

def load_timeseries_data(dataset_name: str):
    """Returns (timeseries, labels, site) as numpy arrays.

    timeseries has shape (N, n_rois, n_timepoints) — (N, 200, 100) for
    abide, (N, 200, 200) for parkinson, (N, 200, 424) for parkinson_a424.
    """
    if dataset_name not in ("abide", "parkinson", "parkinson_a424"):
        raise ValueError(
            f"finetune_brainlm_timeseries.py only supports 'abide', "
            f"'parkinson' and 'parkinson_a424' (got '{dataset_name}') — "
            f"ADNI files don't contain real per-subject timeseries data."
        )
    cfg  = DATASET_CFG[dataset_name]
    path = cfg["path"]
    print(f"Loading {dataset_name} timeseries from {path}")
    data = np.load(path, allow_pickle=True).item()
    ts     = data["timeseires"].astype(np.float32)
    labels = data["label"].astype(int)
    site   = data["site"]
    if not np.any(ts):
        raise RuntimeError(
            f"'{dataset_name}' timeseires array is entirely zero — not real "
            f"data. Refusing to train on it."
        )
    return ts, labels, site


# ===== Timeseries -> image tensor =====

class TimeseriesImageDataset(Dataset):
    """Converts each subject's (n_rois, n_timepoints) timeseries into a
    3-channel square tensor, the input format expected by BrainLM's ViTMAE
    encoder."""
    ...
    def _mat_to_image(self, mat):
        x = torch.tensor(mat, dtype=torch.float32).unsqueeze(0).unsqueeze(0)  # [1,1,ROI,T]
        # mat is already site z-scored (train-fit, via site_zscore_fit).
        # Clamp to +-3 (~99.7% of a normal dist) and use as-is — zero-centered,
        # unit-scale, no further rescaling (same rationale as
        # finetune_brainlm.py's FC normalization).
        x = torch.clamp(x, -3.0, 3.0)
        x = F.interpolate(x, size=(self.image_size, self.image_size),
                          mode="bilinear", align_corners=False)
        # No ImageNet mean/std normalization: BrainLM's ViTMAE was not
...
def load_brainlm_encoder(model_id="", device=None):
    ...
    # Disable ViTMAE's random patch masking (mask_ratio=0.75 by default) so
    # the encoder sees the full image deterministically — masking is part of
    # the MAE pretraining objective, not appropriate for downstream
    # classification.
...
def configure_encoder_trainable(encoder, mode="full", unfreeze_last_k=2):
    ...
    mode="full"   — train every encoder parameter (default).
    ...
    mode="last_k" — freeze everything except the last `unfreeze_last_k`
                    transformer blocks (and the final layernorm).
    """
    if mode == "full":
...
def finetune_brainlm(ts_norm, labels, tr_idx, val_idx,
    ...
    train_ds = TimeseriesImageDataset(ts_norm, labels, tr_idx, image_size=image_size)
    val_ds   = TimeseriesImageDataset(ts_norm, labels, val_idx, image_size=image_size)
...
def get_probs(model, ts_norm, labels, indices, image_size, batch_size=16):
    device = next(model.parameters()).device
    ds = TimeseriesImageDataset(ts_norm, labels, indices, image_size=image_size)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset",
                        choices=["abide", "parkinson", "parkinson_a424"],
                        required=True,
                        help="Dataset to fine-tune on (timeseries-based — "
                             "only abide/parkinson/parkinson_a424 have real "
                             "per-subject timeseries)")
    ...
    # ── Load data ────────────────────────────────────────────
    ts, labels, site = load_timeseries_data(args.dataset)
    print(f"  N={len(labels)}  timeseries shape={ts.shape}  "
          f"pos={cfg['label_pos']}={(labels==1).sum()}  "
          f"neg={cfg['label_neg']}={(labels==0).sum()}")

    tr_idx, val_idx, test_idx, ts_norm = get_split(ts, labels, site, stratify_by)
    ...
    if args.n_trials > 0:
        ...
        def objective(trial):
            ...
            val_labels, val_probs = get_probs(model, ts_norm, labels, val_idx,
    ...
    for seed in args.seeds:
        ...
        print(f"\n{'='*60}")
        print(f" Seed {seed}  [BrainLM (timeseries) fine-tuned  dataset={args.dataset}]")
        print(f"{'='*60}")
        ...
        test_labels, test_probs = get_probs(model, ts_norm, labels, test_idx,
                                            image_size, args.batch_size)
    ...
    print(f"\n{'='*60}")
    print(f" SUMMARY — BrainLM (timeseries) fine-tuned  dataset={args.dataset}  "
    ...
    out_csv = f"results/results_{args.dataset}_brainlm_timeseries_finetune.csv"
    ...
    print(f"  Medical FM | BrainLM (timeseries) | fine-tuned | {args.dataset} | "
